const CACHE='skmedkart-admin-v5.9.13-hybrid-bill-safe';
const ASSETS=['./','./index.html','./admin.js?v=5.9.13-hybrid-bill-safe','./manifest.webmanifest'];
...
self.addEventListener('fetch',e=>{
  ...
  if(url.pathname.endsWith('/admin.js')){
    const versioned=new Request(new URL('./admin.js?v=5.9.13-hybrid-bill-safe',self.registration.scope),{method:'GET',headers:e.request.headers,mode:'same-origin',credentials:e.request.credentials,cache:'no-store'});

const CACHE='skmedkart-admin-v5.9.39-cross-device-restore-stock-fix';
const ASSETS=['./','./index.html','./admin.js?v=5.9.39-cross-device-restore-stock-fix','./manifest.webmanifest','./invoice-top-logo.png','./invoice-footer-logo.jpg','./pharmacist_signature.jpg'];
...
self.addEventListener('fetch',e=>{
  ...
  if(url.pathname.endsWith('/admin.js')){
    const versioned=new Request(new URL('./admin.js?v=5.9.39-cross-device-restore-stock-fix',self.registration.scope),{method:'GET',headers:e.request.headers,mode:'same-origin',credentials:e.request.credentials,cache:'no-store'});

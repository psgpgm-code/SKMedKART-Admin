const CACHE='skmedkart-admin-v5.9.4-minqty-manufacturer-fix';
const ASSETS=['./','./index.html','./admin.js?v=5.9.4-minqty-manufacturer-fix','./manifest.webmanifest'];
...
self.addEventListener('fetch',e=>{
  ...
  if(url.pathname.endsWith('/admin.js')){
    const versioned=new Request(new URL('./admin.js?v=5.9.4-minqty-manufacturer-fix',self.registration.scope),{method:'GET',headers:e.request.headers,mode:'same-origin',credentials:e.request.credentials,cache:'no-store'});

...
window.printBill=id=>{const b=typeof id==='object'?id:findBill(id);if(!b)return alert('Bill not found.');const html=billHtml(b);let frame=document.getElementById('printFrame');if(frame)frame.remove();frame=document.createElement('iframe');frame.id='printFrame';frame.setAttribute('title','Print Invoice '+b.invoiceNumber);frame.style.position='fixed';frame.style.right='0';frame.style.bottom='0';frame.style.width='1px';frame.style.height='1px';frame.style.border='0';frame.style.opacity='0';document.body.appendChild(frame);const d=frame.contentDocument||frame.contentWindow.document;d.open();d.write(html);d.close();const doPrint=()=>{try{frame.contentWindow.focus();frame.contentWindow.print();return true}catch(e){return false}};frame.onload=()=>setTimeout(()=>{if(!doPrint()){const w=window.open('','_blank','noopener,noreferrer');if(!w)return alert('Printing is not available in this app. Please use Save PDF.');w.document.write(html);w.document.close();w.focus();setTimeout(()=>w.print(),250)}},150);setTimeout(()=>doPrint(),800)};
...
window.saveBillPdf=id=>{const b=findBill(id);if(!b)return alert('Bill not found.');const blob=pdfBlob(b),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=(b.invoiceNumber||'bill')+'.pdf';a.rel='noopener';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),3000)};
window.shareBill=async id=>{const b=findBill(id);if(!b)return alert('Bill not found.');const text='Sri Krishna Medicals, Pennagaram\nInvoice: '+b.invoiceNumber+'\nCustomer: '+(b.customerName||'Walk-in Customer')+'\nTotal: '+money(b.grandTotal)+'\nPayment: '+(b.paymentMode||'');if(typeof navigator.share==='function'){try{const file=new File([pdfBlob(b)],(b.invoiceNumber||'bill')+'.pdf',{type:'application/pdf'});if(typeof navigator.canShare==='function'&&navigator.canShare({files:[file]})){await navigator.share({title:'Medical Bill '+b.invoiceNumber,text,files:[file]})}else{await navigator.share({title:'Medical Bill '+b.invoiceNumber,text})}return}catch(e){if(e?.name==='AbortError')return}}try{await navigator.clipboard?.writeText(text)}catch(e){}const wa='https://wa.me/?text='+encodeURIComponent(text);window.location.href=wa};

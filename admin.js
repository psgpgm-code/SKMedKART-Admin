...
async function pdfBlob(b){
  ...
  const html=buildInvoiceHtml(b,assets);
  return new Promise((resolve,reject)=>{
    ...
    const docx=iframe.contentDocument||iframe.contentWindow.document;
    docx.open();docx.write(html);docx.close();
    iframe.onload=()=>setTimeout(()=>{
      try{
        const svg='<svg xmlns="http://www.w3.org/2000/svg" width="794" height="1123"><foreignObject width="100%" height="100%"><div xmlns="http://www.w3.org/1999/xhtml" style="width:794px;height:1123px;overflow:hidden;background:white">'+docx.body.innerHTML+'</div></foreignObject></svg>';
        const img=new Image();
        img.onload=()=>{
          try{
            const c=document.createElement('canvas');c.width=1588;c.height=2246;
            const ctx=c.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,c.width,c.height);ctx.drawImage(img,0,0,c.width,c.height);
            const base64=c.toDataURL('image/jpeg',0.94).split(',')[1],bin=atob(base64),imgBytes=new Uint8Array(bin.length);
            for(let i=0;i<bin.length;i++)imgBytes[i]=bin.charCodeAt(i);
            const enc=new TextEncoder();
            const B=x=>enc.encode(x);
            const content='q\n595 0 0 842 0 0 cm\n/Im1 Do\nQ';
            const objs={
              1:B('<< /Type /Catalog /Pages 2 0 R >>'),
              2:B('<< /Type /Pages /Kids [3 0 R] /Count 1 >>'),
              3:B('<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /XObject << /Im1 4 0 R >> >> /Contents 5 0 R >>'),
              4:B('<< /Type /XObject /Subtype /Image /Width 1588 /Height 2246 /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length '+imgBytes.length+' >>\nstream\n'),
              5:B('<< /Length '+B(content).length+' >>\nstream\n'+content+'\nendstream')
            };
            const chunks=[B('%PDF-1.4\n')];const offs=[0];let pos=chunks[0].length;
            for(let n=1;n<=3;n++){const h=B(n+' 0 obj\n'),t=B('\nendobj\n');offs[n]=pos;chunks.push(h,objs[n],t);pos+=h.length+objs[n].length+t.length}
            const h4=B('4 0 obj\n'),tail4=B('\nendstream\nendobj\n');offs[4]=pos;chunks.push(h4,objs[4],imgBytes,tail4);pos+=h4.length+objs[4].length+imgBytes.length+tail4.length;
            const h5=B('5 0 obj\n'),tail5=B('\nendobj\n');offs[5]=pos;chunks.push(h5,objs[5],tail5);pos+=h5.length+objs[5].length+tail5.length;
            const xrefPos=pos;let xref='xref\n0 6\n0000000000 65535 f \n';for(let n=1;n<=5;n++)xref+=String(offs[n]).padStart(10,'0')+' 00000 n \n';xref+='trailer\n<< /Size 6 /Root 1 0 R >>\nstartxref\n'+xrefPos+'\n%%EOF';chunks.push(B(xref));cleanup();resolve(new Blob(chunks,{type:'application/pdf'}));

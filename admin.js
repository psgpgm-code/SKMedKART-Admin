...
function billHtml(b){
 const discLabel=b.discountType==='percent'?'Discount ('+Number(b.discountRate||0)+'%)':'Discount';
 return '<!doctype html><html><head><meta charset="utf-8"><title>'+esc(b.invoiceNumber)+'</title><style>body{font-family:Arial;padding:20px;max-width:760px;margin:auto;color:#222}table{width:100%;border-collapse:collapse}th,td{border:1px solid #333;padding:7px;text-align:left}.r{text-align:right}.totals{margin-top:14px;border-top:1px solid #333;padding-top:8px}.grand{font-size:20px;font-weight:700}.printBtn{display:block;margin:0 auto 16px;padding:12px 22px;font-size:16px;font-weight:700;border:0;border-radius:8px;cursor:pointer}@media print{.printBtn{display:none}}</style></head><body><button class="printBtn" onclick="window.print()">🖨 Print Bill</button>'+shopHeaderHtml()+'<h3 style="text-align:center;margin:8px 0">Invoice '+esc(b.invoiceNumber)+'</h3><p><b>Date:</b> '+esc(b.billDate)+'<br><b>Customer:</b> '+esc(b.customerName)+'<br><b>Mobile:</b> '+esc(b.mobile||'-')+'<br><b>Prescribed By:</b> '+esc(b.doctor||'-')+'</p><table><tr><th>Medicine</th><th>Batch</th><th>Qty</th><th>Rate</th><th>Amount</th></tr>'+b.items.map(i=>'<tr><td>'+esc(i.name)+'</td><td>'+esc(i.batchNumber)+'</td><td>'+i.qty+'</td><td>'+money(i.price)+'</td><td>'+money(i.qty*i.price)+'</td></tr>').join('')+'</table><div class="totals">Subtotal: '+money(b.subtotal)+'<br>'+discLabel+': -'+money(b.discount)+'<br>GST: '+money(b.gst)+'<br><span class="grand">Grand Total: '+money(b.grandTotal)+'</span></div><p><b>Payment:</b> '+esc(b.paymentMode)+'<br><b>Note:</b> '+esc(b.note||'-')+'</p></body></html>'
}
...
window.printBill=id=>{
  ...
  // Open the print page immediately from the button click so mobile browsers do not block it.
  const w=window.open('about:blank','_blank');
  if(!w)return alert('Printing window was blocked. Please allow pop-ups for this app.');
  const html=billHtml(b).replace('</body>','<script>window.addEventListener(\"load\",function(){setTimeout(function(){try{window.focus();window.print()}catch(e){}},700)});<\/script></body>');
...
window.shareBill=async id=>{
  const b=findBill(id);
  if(!b)return alert('Bill not found.');
  const items=(b.items||[]).map((i,n)=>`${n+1}. ${i.name||i.productName||'-'} | Qty: ${i.qty||0} | ${money(Number(i.qty||0)*Number(i.price||0))}`).join('\n');
  const discText=b.discountType==='percent'?`Discount (${Number(b.discountRate||0)}%): -${money(b.discount)}`:`Discount: -${money(b.discount)}`;
  const text=`Sri Krishna Medicals, Pennagaram\n\n🧾 BILL COPY\nBill No: ${b.invoiceNumber}\nDate: ${b.billDate||'-'}\nCustomer: ${b.customerName||'Walk-in Customer'}\nMobile: ${b.mobile||'-'}\n\n${items}\n\nSubtotal: ${money(b.subtotal)}\n${discText}\nGST: ${money(b.gst)}\nGrand Total: ${money(b.grandTotal)}\nPayment: ${b.paymentMode||'-'}\n\nThank you - Sri Krishna Medicals`;
  try{
    // On supported Android browsers, share the actual PDF bill so WhatsApp can receive it as a file.
    const blob=pdfBlob(b);
    const file=new File([blob],`${b.invoiceNumber}.pdf`,{type:'application/pdf'});
    if(navigator.share && (!navigator.canShare || navigator.canShare({files:[file]}))){
      await navigator.share({title:`Bill ${b.invoiceNumber}`,text:'Sri Krishna Medicals - Bill Copy',files:[file]});
      return;
    }
  }catch(e){
    if(e?.name==='AbortError')return;
    console.warn('PDF share unavailable, using WhatsApp text fallback:',e);
  }
  // Fallback: open the customer's WhatsApp chat with the complete bill copy.
  const phone=String(b.mobile||'').replace(/\D/g,'');
  const waPhone=phone.length===10?'91'+phone:(phone.length>=11?phone:'');
  const waUrl=waPhone?`https://wa.me/${waPhone}?text=${encodeURIComponent(text)}`:`https://wa.me/?text=${encodeURIComponent(text)}`;

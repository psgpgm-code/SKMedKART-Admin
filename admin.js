...
async function loadAll(force=false){
 ...
 products=get('products',products);currentOrders=get('orders',currentOrders);purchases=get('purchases',purchases);batches=get('batches',batches);bills=get('bills',bills);customers=get('customers',customers);reminders=get('reminders',reminders);suppliers=get('suppliers',suppliers);
 liveStarted=true;
...
function purchaseRow(p){return '<div class="itemrow"><b>'+esc(p.productName||p.medicine||p.name||'-')+'</b> • Qty '+Number(p.qty||0)+'<br><span class="small">'+(p.manufacturer||p.manufacturerDetails?('Manufacturer: '+esc(p.manufacturer||p.manufacturerDetails)+' • '):'')+esc(p.supplier||p.supplierName||'-')+' • Batch '+esc(p.batchNumber||p.batch||'-')+' • Exp '+esc(p.expiryDate||'-')+' • Purchase ₹'+Number(p.purchasePrice||0).toFixed(2)+' + GST '+Number(p.purchaseGstRate||0).toFixed(2)+'% = ₹'+Number(p.purchasePriceWithGst??(Number(p.purchasePrice||0)*(1+Number(p.purchaseGstRate||0)/100))).toFixed(2)+'</span></div>'}
...
function reportHtml(){const r=window.__reportRange||{from:'',to:'',rows:[]},title=r.from&&r.to?('Date-wise Sales Report: '+r.from+' to '+r.to):'Sales Report';const rows=r.rows||[];return '<!doctype html><html><head><meta charset="utf-8"><title>'+esc(title)+'</title><style>@page{size:A4;margin:12mm}body{font-family:Arial,sans-serif;color:#222}h1{color:#174b82;font-size:22px}h2{font-size:15px;margin:4px 0 14px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #999;padding:7px;font-size:11px;text-align:left}th{background:#dff0ff;color:#123f7c}.totals{margin-top:16px;font-size:13px;line-height:1.7}.grand{font-size:16px;font-weight:bold;color:#174b82}footer{margin-top:20px;border-top:2px solid #174b82;padding-top:7px;font-size:10px}</style></head><body><h1>Sri Krishna Medicals</h1><h2>'+esc(title)+'</h2><table><thead><tr><th>S.No</th><th>Invoice</th><th>Customer</th><th>Date</th><th>Total</th><th>Payment</th></tr></thead><tbody>'+rows.map((b,i)=>'<tr><td>'+(i+1)+'</td><td>'+esc(b.invoiceNumber||'-')+'</td><td>'+esc(b.customerName||'Walk-in Customer')+'</td><td>'+esc(reportDateValue(b.billDate||b.date||b.createdAt))+'</td><td>'+money(b.grandTotal)+'</td><td>'+esc(b.paymentMode||'-')+'</td></tr>').join('')+'</tbody></table><div class="totals"><div>Total Sales: <b>'+money(r.sales)+'</b></div><div>Purchase Value: <b>'+money(r.purchaseCost)+'</b></div><div class="grand">Gross Margin: '+money(r.gross)+'</div><div>Bills: <b>'+rows.length+'</b></div></div><footer>Generated from SKMedKART • Sri Krishna Medicals, Pennagaram</footer></body></html>'}
window.printDateReport=()=>{const r=window.__reportRange;if(!r?.from||!r?.to)return alert('Select a report period first.');const w=window.open('','_blank');if(!w)return alert('Please allow popups for printing.');w.document.write(reportHtml());w.document.close();w.focus();setTimeout(()=>w.print(),300)};
async function reportPdfBlob(){const r=window.__reportRange;if(!r?.from||!r?.to)throw new Error('Select a report period first.');const rows=r.rows||[],W=1240,H=1754,pages=[];const chunksFor=arr=>{const canvas=document.createElement('canvas');canvas.width=W;canvas.height=H;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,W,H);ctx.strokeStyle='#0b5aa6';ctx.lineWidth=2;ctx.strokeRect(10,10,W-20,H-20);ctx.fillStyle='#123f7c';ctx.font='700 34px Arial';ctx.fillText('Sri Krishna Medicals',45,65);ctx.font='22px Arial';ctx.fillStyle='#222';ctx.fillText('Kaveri Road, Pennagaram, Dharmapuri District, Tamil Nadu - 636 810',45,98);ctx.font='700 26px Arial';ctx.fillStyle='#123f7c';ctx.fillText('Date-wise / Monthly Sales Report',45,145);ctx.font='19px Arial';ctx.fillStyle='#222';ctx.fillText('Period: '+r.from+' to '+r.to,45,178);const x=[45,145,390,700,900,1040,1195],top=215,headH=42,rowH=42;ctx.fillStyle='#dff0ff';ctx.fillRect(x[0],top,x[6]-x[0],headH);ctx.strokeStyle='#555';ctx.lineWidth=1;ctx.strokeRect(x[0],top,x[6]-x[0],headH);['S.No','Invoice','Customer','Date','Total','Payment'].forEach((h,i)=>{ctx.fillStyle='#123f7c';ctx.font='700 16px Arial';ctx.fillText(h,x[i]+8,top+27);});let y=top+headH;arr.forEach((b,n)=>{ctx.strokeStyle='#777';ctx.strokeRect(x[0],y,x[6]-x[0],rowH);for(let j=1;j<x.length-1;j++){ctx.beginPath();ctx.moveTo(x[j],y);ctx.lineTo(x[j],y+rowH);ctx.stroke()}const vals=[String(n+1),String(b.invoiceNumber||'-'),String(b.customerName||'Walk-in Customer'),reportDateValue(b.billDate||b.date||b.createdAt),money(b.grandTotal),String(b.paymentMode||'-')];vals.forEach((v,j)=>{ctx.fillStyle='#222';ctx.font='15px Arial';ctx.fillText(v.slice(0,j===2?31:18),x[j]+8,y+27)});y+=rowH});if(arr.length&&y<1400){ctx.fillStyle='#222';ctx.font='700 19px Arial';ctx.fillText('Total Sales',55,y+45);ctx.font='19px Arial';ctx.fillText(money(r.sales),220,y+45);ctx.font='700 19px Arial';ctx.fillText('Purchase Value',55,y+78);ctx.font='19px Arial';ctx.fillText(money(r.purchaseCost),220,y+78);ctx.font='700 21px Arial';ctx.fillStyle='#123f7c';ctx.fillText('Gross Margin: '+money(r.gross),55,y+115);ctx.fillStyle='#222';ctx.font='19px Arial';ctx.fillText('Bills: '+rows.length,55,y+148)}ctx.fillStyle='#0b4f91';ctx.fillRect(10,H-72,W-20,52);ctx.fillStyle='#fff';ctx.font='15px Arial';ctx.fillText('SKMedKART • Sri Krishna Medicals, Pennagaram',35,H-40);return canvas};for(let i=0;i<rows.length||i===0;i+=25){pages.push(chunksFor(rows.slice(i,i+25)));if(i+25>=rows.length)break}const enc=new TextEncoder(),B=x=>enc.encode(String(x)),objs=[],chunks=[B('%PDF-1.4\n')],offs=[0],images=[];let pos=chunks[0].length;pages.forEach((canvas,pi)=>{const raw=atob(canvas.toDataURL('image/jpeg',0.9).split(',')[1]),u=new Uint8Array(raw.length);for(let i=0;i<raw.length;i++)u[i]=raw.charCodeAt(i);images.push(u)});const pageCount=pages.length,basePage=3,baseImg=3+pageCount*2,baseContent=baseImg+pageCount;objs.push(B('<< /Type /Catalog /Pages 2 0 R >>'));objs.push(null);const kids=[];for(let i=0;i<pageCount;i++)kids.push((basePage+i*2)+' 0 R');objs[1]=B('<< /Type /Pages /Kids ['+kids.join(' ')+'] /Count '+pageCount+' >>');for(let i=0;i<pageCount;i++){const pageNo=basePage+i*2,imgNo=baseImg+i,contNo=baseContent+i;objs.push(B('<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /XObject << /Im'+(i+1)+' '+imgNo+' 0 R >> >> /Contents '+contNo+' 0 R >>'));objs.push(B('<< /Length 29 >>\nstream\nq 595 0 0 842 0 0 cm /Im'+(i+1)+' Do Q\nendstream'));}for(let i=0;i<pageCount;i++){const u=images[i];objs.push(B('<< /Type /XObject /Subtype /Image /Width '+W+' /Height '+H+' /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length '+u.length+' >>\nstream\n'+''));objs.push(null)}// rebuild object list cleanly with exact object numbering
const all=[null];const put=n=>{while(all.length<=n)all.push(null);};put(1);all[1]=B('<< /Type /Catalog /Pages 2 0 R >>');put(2);all[2]=B('<< /Type /Pages /Kids ['+kids.join(' ')+'] /Count '+pageCount+' >>');for(let i=0;i<pageCount;i++){const pageNo=basePage+i*2,imgNo=baseImg+i,contNo=baseContent+i;put(pageNo);all[pageNo]=B('<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /XObject << /Im'+(i+1)+' '+imgNo+' 0 R >> >> /Contents '+contNo+' 0 R >>');put(contNo);const content='q 595 0 0 842 0 0 cm /Im'+(i+1)+' Do Q';all[contNo]=B('<< /Length '+content.length+' >>\nstream\n'+content+'\nendstream');put(imgNo);all[imgNo]=images[i];}let max=all.length-1;const typeMap={};for(let i=1;i<=max;i++){offs[i]=pos;const head=B(i+' 0 obj\n');chunks.push(head);pos+=head.length;if(typeMap[i]==='image'){}if(i>=baseImg&&i<baseImg+pageCount){const u=all[i],headImg=B('<< /Type /XObject /Subtype /Image /Width '+W+' /Height '+H+' /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length '+u.length+' >>\nstream\n');chunks.push(headImg,u,B('\nendstream\nendobj\n'));pos+=headImg.length+u.length+16}else{const body=all[i]||B('');chunks.push(body,B('\nendobj\n'));pos+=body.length+9}}const xref=pos;let xs='xref\n0 '+(max+1)+'\n0000000000 65535 f \n';for(let i=1;i<=max;i++)xs+=String(offs[i]||0).padStart(10,'0')+' 00000 n \n';xs+='trailer\n<< /Size '+(max+1)+' /Root 1 0 R >>\nstartxref\n'+xref+'\n%%EOF';chunks.push(B(xs));return new Blob(chunks,{type:'application/pdf'})}
window.saveDateReportPdf=async()=>{const r=window.__reportRange;if(!r?.from||!r?.to)return alert('Select a report period first.');try{const blob=await reportPdfBlob(),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='SKMedKART-Report-'+r.from+'-to-'+r.to+'.pdf';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}catch(e){alert('Could not create report PDF: '+e.message)}};

...
window.printBill=id=>{
 ...
 const cleanup=()=>setTimeout(()=>{area.innerHTML=''},1500);
 try{window.focus();setTimeout(()=>{try{window.print();cleanup()}catch(e){cleanup();const w=window.open('','_blank');if(!w)return alert('Printing is not available in this installed app. Please use Save PDF.');w.document.open();w.document.write(html);w.document.close();w.focus();setTimeout(()=>w.print(),300)}},50)}catch(e){cleanup();alert('Printing is not available. Please use Save PDF.')}
};
function pdfEscape(s){return String(s??'').replace(/\\/g,'\\\\').replace(/[()]/g,'\\$&').replace(/[^\x20-\x7E]/g,'?')}
function pdfBlob(b){const lines=['Sri Krishna Medicals','Kaveri Road, Pennagaram, Dharmapuri District, Tamil Nadu','Phone: 8300363317','Drug Licence No: TN/DPI/01386/20,21','FSSAI Licence No: 224220390000512','','Invoice: '+b.invoiceNumber,'Date: '+b.billDate,'Customer: '+b.customerName,'Mobile: '+(b.mobile||'-'),'Prescribed By: '+(b.doctor||'-'),'','MEDICINE / BATCH / QTY / AMOUNT',...(b.items||[]).map(i=>i.name+' / '+i.batchNumber+' / '+i.qty+' / '+money(i.qty*i.price)),'','Subtotal: '+money(b.subtotal),'Discount: '+money(b.discount),'GST: '+money(b.gst),'Grand Total: '+money(b.grandTotal),'Payment: '+b.paymentMode];const text=['BT','/F1 10 Tf','50 800 Td',...lines.flatMap((l,i)=>[i?'0 -16 Td':'','('+pdfEscape(l)+') Tj']).filter(Boolean),'ET'].join('\n');const objs=['<< /Type /Catalog /Pages 2 0 R >>','<< /Type /Pages /Kids [3 0 R] /Count 1 >>','<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 4 0 R >> >> /Contents 5 0 R >>','<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>','<< /Length '+text.length+' >>\nstream\n'+text+'\nendstream'];let pdf='%PDF-1.4\n',offset=[0];objs.forEach((o,i)=>{offset.push(pdf.length);pdf+=(i+1)+' 0 obj\n'+o+'\nendobj\n'});const x=pdf.length;pdf+='xref\n0 '+(objs.length+1)+'\n0000000000 65535 f \n'+offset.slice(1).map(n=>String(n).padStart(10,'0')+' 00000 n \n').join('')+'trailer\n<< /Size '+(objs.length+1)+' /Root 1 0 R >>\nstartxref\n'+x+'\n%%EOF';return new Blob([pdf],{type:'application/pdf'})}
function downloadBlob(blob,name){const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=name;a.target='_blank';a.rel='noopener';a.style.display='none';document.body.appendChild(a);try{a.click()}catch(e){}setTimeout(()=>{a.remove();URL.revokeObjectURL(url)},5000)}
window.saveBillPdf=id=>{const b=typeof id==='object'?id:findBill(id);if(!b)return alert('Bill not found.');const blob=pdfBlob(b);try{downloadBlob(blob,(b.invoiceNumber||'SKMedKART-Bill')+'.pdf')}catch(e){const url=URL.createObjectURL(blob);location.href=url;setTimeout(()=>URL.revokeObjectURL(url),10000)}};
window.shareBill=async id=>{const b=typeof id==='object'?id:findBill(id);if(!b)return alert('Bill not found.');const text='Sri Krishna Medicals, Pennagaram\nInvoice: '+b.invoiceNumber+'\nCustomer: '+(b.customerName||'Walk-in Customer')+'\nTotal: '+money(b.grandTotal)+'\nPayment: '+(b.paymentMode||'Cash');const blob=pdfBlob(b);try{if(typeof File!=='undefined'&&navigator.share){const file=new File([blob],(b.invoiceNumber||'SKMedKART-Bill')+'.pdf',{type:'application/pdf'});if(navigator.canShare?.({files:[file]})){await navigator.share({title:'Medical Bill '+b.invoiceNumber,text,files:[file]});return}await navigator.share({title:'Medical Bill '+b.invoiceNumber,text});return}}catch(e){if(e?.name==='AbortError')return;console.warn('Web Share failed:',e)}try{if(navigator.clipboard?.writeText)await navigator.clipboard.writeText(text)}catch(e){}const wa='https://wa.me/?text='+encodeURIComponent(text);try{location.href=wa}catch(e){alert('Bill details copied. Please paste them in WhatsApp.')}};
...
if(!window.__skmDeletePwaHandler){window.__skmDeletePwaHandler=true;document.addEventListener('click',e=>{if(e.__skmDeleteHandled)return;const btn=e.target.closest?.('[data-delete-product]');if(!btn)return;e.__skmDeleteHandled=true;e.preventDefault();e.stopImmediatePropagation();const id=btn.getAttribute('data-delete-product');if(id)window.deleteProduct(id)},true)}

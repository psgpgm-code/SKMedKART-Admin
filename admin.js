...
window.printBill=id=>{const b=typeof id==='object'?id:findBill(id);if(!b)return alert('Bill not found.');const html=billHtml(b);const openPrintWindow=()=>{const w=window.open('about:blank','_blank');if(!w)return false;try{w.document.open();w.document.write(html);w.document.close();w.focus();setTimeout(()=>{try{w.print()}catch(e){}},350);return true}catch(e){try{w.close()}catch(_e){}return false}};try{let frame=document.getElementById('printFrame');if(frame)frame.remove();frame=document.createElement('iframe');frame.id='printFrame';frame.style.position='fixed';frame.style.left='-10000px';frame.style.top='0';frame.style.width='800px';frame.style.height='1000px';frame.style.border='0';document.body.appendChild(frame);const docx=frame.contentDocument||frame.contentWindow.document;docx.open();docx.write(html);docx.close();setTimeout(()=>{let ok=false;try{frame.contentWindow.focus();frame.contentWindow.print();ok=true}catch(e){}if(!ok)openPrintWindow()},300);setTimeout(()=>{try{frame.remove()}catch(e){}},5000)}catch(e){if(!openPrintWindow())alert('Please allow popups for printing.')}};
...
window.saveBillPdf=id=>{const b=findBill(id);if(!b)return alert('Bill not found.');const blob=pdfBlob(b),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=(b.invoiceNumber||'bill')+'.pdf';a.target='_blank';a.rel='noopener';a.style.display='none';document.body.appendChild(a);try{a.click()}catch(e){window.open(url,'_blank')}setTimeout(()=>{a.remove();URL.revokeObjectURL(url)},5000)};
window.shareBill=async id=>{const b=findBill(id);if(!b)return alert('Bill not found.');const text='Sri Krishna Medicals, Pennagaram\nInvoice: '+b.invoiceNumber+'\nCustomer: '+b.customerName+'\nTotal: '+money(b.grandTotal)+'\nPayment: '+b.paymentMode;try{if(typeof navigator!=='undefined'&&typeof navigator.share==='function'){const file=new File([pdfBlob(b)],(b.invoiceNumber||'bill')+'.pdf',{type:'application/pdf'});if(typeof navigator.canShare==='function'&&navigator.canShare({files:[file]})){await navigator.share({title:'Medical Bill '+b.invoiceNumber,text,files:[file]});return}await navigator.share({title:'Medical Bill '+b.invoiceNumber,text});return}}catch(e){if(e?.name==='AbortError')return}try{if(navigator.clipboard?.writeText)await navigator.clipboard.writeText(text)}catch(e){}try{window.location.assign('https://api.whatsapp.com/send?text='+encodeURIComponent(text))}catch(e){alert('Bill details copied. You can paste and send to the customer.')}};

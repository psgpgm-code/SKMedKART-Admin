...
window.cancelOrder=async id=>{
 ...
 if(!confirm('Cancel this customer order? This will stop the order from being billed.'))return;
 try{
   const note=(o.pharmacistNote||'').trim();
   const updatedNote=(note?note+'\n':'')+'Order cancelled by admin.';
   if(configured)await updateDoc(doc(db,'orders',id),{status:'Cancelled',pharmacistNote:updatedNote,cancelledAt:serverTimestamp(),updatedAt:serverTimestamp()});
   else{Object.assign(o,{status:'Cancelled',pharmacistNote:updatedNote,cancelledAt:new Date().toISOString()});set('orders',currentOrders);renderOrders();}
   alert('Order cancelled successfully.');
 }catch(e){alert('Could not cancel order: '+e.message)}
};
window.updateOrder=async id=>{const existing=currentOrders.find(x=>x.id===id);if(existing&&existing.status==='Cancelled')return alert('Cancelled order cannot be changed or billed.');const st=$('st_'+id).value,note=$('note_'+id).value.trim();try{if(configured)await updateDoc(doc(db,'orders',id),{status:st,pharmacistNote:note,updatedAt:serverTimestamp()});else{const o=currentOrders.find(x=>x.id===id);if(o){o.status=st;o.pharmacistNote=note;set('orders',currentOrders)}}alert('Order status updated.')}catch(e){alert(e.message)}};

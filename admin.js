...
async function loadAll(force=false){if(!configured){products=get('products',[]);currentOrders=get('orders',[]);purchases=get('purchases',[]);batches=get('batches',[]);bills=get('bills',[]);customers=get('customers',[]);reminders=get('reminders',[]);suppliers=get('suppliers',[]);renderAll();return}if(liveStarted&&!force){renderAll();return}if(force){location.reload();return}liveStarted=true;
 const showSyncError=(name,e)=>{console.error('Firebase '+name+' error:',e);const n=$('notice');if(n)n.innerHTML='<b>⚠️ Firebase '+esc(name)+' sync error</b><br><span class="small">'+esc(e.message||'Please check Firebase rules and refresh.')+'</span>'};
 const listen=(name,assign)=>onSnapshot(collection(db,name),s=>{assign(s.docs.map(d=>({id:d.id,...d.data()})));try{renderAll()}catch(e){console.error('Render '+name+' error:',e);if(name==='orders')try{renderOrders();renderDashboard()}catch(_){} }},e=>showSyncError(name,e));
 // Orders get their own initial read + realtime listener so the Orders panel is not dependent on other view rendering.
 try{const snap=await getDocs(collection(db,'orders'));currentOrders=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>t(b.createdAt)-t(a.createdAt));try{renderOrders();renderDashboard()}catch(e){console.error('Initial orders render error:',e)}}catch(e){showSyncError('orders',e)}
 onSnapshot(collection(db,'orders'),s=>{currentOrders=s.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>t(b.createdAt)-t(a.createdAt));try{renderOrders();renderDashboard()}catch(e){console.error('Orders render error:',e)}},e=>showSyncError('orders',e));

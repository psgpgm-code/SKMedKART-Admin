...
function renderPurchases(){const input=$('purchaseSearch'),q=String(input?.value||'').trim().toLowerCase();const rows=purchases.filter(p=>{if(!q)return true;const prod=products.find(x=>x.id===p.productId);const text=[p.productName,p.medicine,p.name,p.supplier,p.supplierName,p.invoice,p.invoiceNo,p.batchNumber,p.batch,p.expiryDate,prod?.name,prod?.barcode].map(v=>String(v??'')).join(' ').toLowerCase();return text.includes(q)}).slice(0,100);if(!input)return;if(!q){$('purchases').innerHTML=rows.map(p=>purchaseRow(p)).join('')||'<div class="small">No purchases found.</div>';return}if(rows.length){$('purchases').innerHTML=rows.map(p=>purchaseRow(p)).join('');return}const matches=products.filter(p=>[p.name,p.barcode].map(v=>String(v??'')).join(' ').toLowerCase().includes(q)).slice(0,30);$('purchases').innerHTML=matches.map(p=>'<div class="itemrow"><b>'+esc(p.name||'-')+'</b><br><span class="small">Current stock: '+Number(p.stock||0)+' • Search result from Stock</span></div>').join('')||'<div class="small">No purchases or products found.</div>'}

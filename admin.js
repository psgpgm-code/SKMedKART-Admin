...
window.shareBill=async id=>{const b=findBill(id);if(!b)return alert('Bill not found.');const text='Sri Krishna Medicals, Pennagaram\nInvoice: '+b.invoiceNumber+'\nCustomer: '+b.customerName+'\nTotal: '+money(b.grandTotal)+'\nPayment: '+b.paymentMode;const wa='https://wa.me/?text='+encodeURIComponent(text);try{if(typeof navigator.share==='function'){const file=new File([pdfBlob(b)],b.invoiceNumber+'.pdf',{type:'application/pdf'});if(typeof navigator.canShare==='function'&&navigator.canShare({files:[file]})){await navigator.share({title:'Medical Bill '+b.invoiceNumber,text,files:[file]});return}await navigator.share({title:'Medical Bill '+b.invoiceNumber,text});return}}catch(e){if(e&&e.name==='AbortError')return}try{window.location.href=wa}catch(e){try{const a=document.createElement('a');a.href=wa;a.target='_blank';a.rel='noopener';document.body.appendChild(a);a.click();a.remove()}catch(_){alert('WhatsApp could not be opened. Please use the Share option from your browser.')}}};

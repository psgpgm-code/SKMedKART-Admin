...
function movementDeltaForBatch(batchId){
 const id=String(batchId||'');
 return get('stockMovements',[]).filter(m=>String(m.batchId||'')===id).reduce((n,m)=>n+Number(m.qty||0),0);
}
function hasPurchaseMovementForBatch(batchId){
 const id=String(batchId||'');
...
// V5.9.56 PURCHASE-HISTORY SAFE RECOVERY
// Recover only a purchase ledger row that has an unambiguous original PURCHASE
// stock movement. Never invent a purchase from product/batch stock alone.
function repairLocalStockConsistency(){
 // V5.9.66 ROOT LEDGER RECONCILIATION
 // Purchase History is read-only during reconciliation.
 // NEVER invent, recover, overwrite, or re-date a purchase record.
 // NEVER use invoiceNumber as a transaction identity.
 // For a purchase-backed batch, current stock = purchase ledger - genuine
 // bill records + explicit stock adjustments. For a batch with no purchase
 // ledger, preserve its existing physical/opening stock.
 ...
 const indexBatch=b=>{
   ...
   const pid=String(b?.productId||''),pn=norm(b?.productName||b?.medicineName||b?.medicine||b?.name);
   if(pid){
     const k=pid+'__'+bn;
     if(!batchByPidName.has(k))batchByPidName.set(k,[]);
     batchByPidName.get(k).push(b);
   }
   if(pn){
     const k=pn+'__'+bn;
     if(!batchByName.has(k))batchByName.set(k,[]);
     batchByName.get(k).push(b);
   }
 };
 batches.forEach(indexBatch);
 const resolveBatch=(prod,bn,bid)=>{
   const wanted=norm(bn),byId=batchById.get(String(bid||''));
   if(byId && (!wanted||norm(byId.batchNumber||byId.batch)===wanted) &&
      (String(byId.productId||'')===String(prod?.id||'')||norm(byId.productName)===norm(prod?.name))) return byId;
   if(!wanted)return null;
   const exact=batchByPidName.get(String(prod.id)+'__'+wanted)||[];
   if(exact.length===1)return exact[0];
   const byName=batchByName.get(norm(prod.name)+'__'+wanted)||[];
   return byName.length===1?byName[0]:null;
 };

 // INBOUND: use only the purchase rows that already exist.
 // Missing purchase rows are NOT reconstructed from stock movements.
 ...
 for(const pu of purchases){
   ...
   const b=resolveBatch(prod,pu?.batchNumber||pu?.batch,pu?.batchId||'');if(!b)continue;
   const bid=String(b.id);
   purchaseQtyByBatch.set(bid,(purchaseQtyByBatch.get(bid)||0)+q);
   const pid=String(prod.id);
   if(!purchaseBatchesByProduct.has(pid))purchaseBatchesByProduct.set(pid,[]);
   if(!purchaseBatchesByProduct.get(pid).includes(b))purchaseBatchesByProduct.get(pid).push(b);
 }
 for(const arr of purchaseBatchesByProduct.values()){
   arr.sort((a,b)=>String(a.expiryDate||'').localeCompare(String(b.expiryDate||''))||String(a.batchNumber||'').localeCompare(String(b.batchNumber||'')));
 }

 // OUTBOUND: every bill record is unique by bill.id.
 // Invoice number is only a display/reference number and may never be used
 // to merge two bills.
 const billGroups=new Map();
 for(const bill of bills){
   const id=String(bill?.id||'');
   if(id && !billGroups.has(id))billGroups.set(id,bill);
 }
 const saleQtyByBatch=new Map();
 const addSale=(b,q)=>{
   if(!b||q<=0)return;
   const id=String(b.id);
   saleQtyByBatch.set(id,(saleQtyByBatch.get(id)||0)+q);
 };
 for(const bill of billGroups.values()){
   ...
   for(const it of (bill?.items||[])){
     ...
     if(!prod)continue;
     const b=resolveBatch(prod,it?.batchNumber||it?.batch,it?.batchId||'');
     if(b){addSale(b,q);continue}

     // Legacy bill without a usable batch link: allocate only against
     // purchase-backed batches, FEFO, and never above the recorded inbound.
     let remaining=q;
     for(const candidate of (purchaseBatchesByProduct.get(String(prod.id))||[])){
       if(remaining<=0)break;
       const bid=String(candidate.id);
       const inbound=Number(purchaseQtyByBatch.get(bid)||0);
       const already=Number(saleQtyByBatch.get(bid)||0);
       const take=Math.min(Math.max(0,inbound-already),remaining);
       if(take>0){addSale(candidate,take);remaining-=take}
     }
   }
 }

 // EXPLICIT STOCK ADJUSTMENTS only.
 const adjustmentByBatch=new Map();
 for(const m of sm){
   const type=String(m?.type||'').toUpperCase();
   if(type!=='STOCK_ADJUSTMENT'&&type!=='ADJUSTMENT')continue;
   const prod=resolveProduct(m?.productId,m?.productName||m?.medicine);
   const b=prod?resolveBatch(prod,m?.batchNumber||m?.batch,m?.batchId||''):null;
   if(!b)continue;
   const bid=String(b.id);
   adjustmentByBatch.set(bid,(adjustmentByBatch.get(bid)||0)+Number(m?.qty||0));
 }

 // DERIVE CURRENT STOCK.
 const expectedByProduct=new Map();
 for(const b of batches){
   const bid=String(b?.id||'');if(!bid)continue;
   const inbound=Number(purchaseQtyByBatch.get(bid)||0);
   const hasPurchase=purchaseQtyByBatch.has(bid);
   const sales=Number(saleQtyByBatch.get(bid)||0);
   const adjustments=Number(adjustmentByBatch.get(bid)||0);
   const current=Math.max(0,Number(b?.stock||0));

   let expected;
   if(hasPurchase){
     // Explicit opening stock is honoured only when the record explicitly
     // identifies it. We never infer opening stock from a corrupted current value.
     const opening=(b?.openingStockSource==='MANUAL'||b?.openingStockSource==='OPENING_STOCK')
       ? Math.max(0,Number(b?.openingStock||0)) : 0;
     expected=Math.max(0,opening+inbound-sales+adjustments);

     // Keep an integrity flag when sales exceed the complete inbound ledger.
     if(sales>opening+inbound+adjustments){
       b.stockIntegrityStatus='SALE_EXCEEDS_LEDGER';
       b.stockIntegrityExpected=opening+inbound-sales+adjustments;
       b.stockIntegrityDifference=current-(opening+inbound-sales+adjustments);
     }else{
       delete b.stockIntegrityStatus;
       delete b.stockIntegrityExpected;
       delete b.stockIntegrityDifference;
     }
   }else{
     // No purchase ledger: this is opening/unlinked stock. Do not guess.
     expected=current;
   }

   b.stock=expected;
   const pid=String(b.productId||'');
   if(pid)expectedByProduct.set(pid,(expectedByProduct.get(pid)||0)+expected);
 }

 // Product master stock is the sum of its actual batch stocks.
 // Never merge different product IDs by medicine name.
 for(const p of products){
   const pid=String(p?.id||'');
   if(!pid||!expectedByProduct.has(pid))continue;
   p.stock=Math.max(0,Number(expectedByProduct.get(pid)||0));
 }
 set('batches',batches);
...
window.deletePurchase=async id=>{
 ...
 try{
   ...
   set('stockMovements',allMoves.filter(m=>String(m.id)!==String(purchase.id+'_SM')));
    if(batch){
      batch.stock=Math.max(0,Number(batch.stock||0)-qty);
      if(product)product.stock=batches.filter(x=>String(x.productId)===String(product.id)).reduce((n,x)=>n+Math.max(0,Number(x.stock||0)),0);

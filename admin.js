...
window.printBill=id=>{const b=typeof id==='object'?id:findBill(id);if(!b)return alert('Bill not found.');const html=billHtml(b);let w=null;try{w=window.open('','_blank');}catch(e){}if(w){try{w.document.open();w.document.write(html);w.document.close();w.focus();setTimeout(()=>{try{w.print()}catch(e){alert('Printing is not available in this browser. Please use Save PDF.')}},400);return}catch(e){try{w.close()}catch(_){}}}let frame=document.getElementById('printFrame');if(frame)frame.remove();frame=document.createElement('iframe');frame.id='printFrame';frame.style.position='fixed';frame.style.left='-9999px';frame.style.width='1px';frame.style.height='1px';frame.style.border='0';document.body.appendChild(frame);const docx=frame.contentDocument||frame.contentWindow.document;docx.open();docx.write(html);docx.close();setTimeout(()=>{try{frame.contentWindow.focus();frame.contentWindow.print()}catch(e){alert('Please allow popups for printing, or use Save PDF.')}},500)};
...
window.saveBillPdf=id=>{const b=findBill(id);if(!b)return alert('Bill not found.');try{const blob=pdfBlob(b),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=(b.invoiceNumber||'invoice')+'.pdf';a.rel='noopener';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),3000)}catch(e){alert('Could not create PDF: '+(e.message||e))}};
window.shareBill=async id=>{const b=findBill(id);if(!b)return alert('Bill not found.');const text='Sri Krishna Medicals, Pennagaram\nInvoice: '+(b.invoiceNumber||'-')+'\nCustomer: '+(b.customerName||'Walk-in Customer')+'\nTotal: '+money(b.grandTotal)+'\nPayment: '+(b.paymentMode||'Cash');try{if(navigator.share){const file=new File([pdfBlob(b)],(b.invoiceNumber||'invoice')+'.pdf',{type:'application/pdf'});if(navigator.canShare?.({files:[file]})){await navigator.share({title:'Medical Bill '+b.invoiceNumber,text,files:[file]});return}await navigator.share({title:'Medical Bill '+b.invoiceNumber,text});return}}catch(e){if(e?.name==='AbortError')return}try{await navigator.clipboard.writeText(text);alert('Bill details copied. Paste and send to the customer. You can use Save PDF to attach the bill.')}catch(e){alert(text)}};

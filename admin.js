...
function getOrderReservedItems(o){
 const candidates=[o.stockDeductedItems,o.reservedItems,o.stockReservedItems,o.allocatedItems];
 ...
 return (o.items||[]).filter(x=>x.stockReserved===true||x.stockDeducted===true||x.reserved===true).map(x=>({...x,qty:Math.max(0,Number(x.qty??x.quantity??0))})).filter(x=>x.qty>0);
}
async function restoreCancelledOrderStock(id,o,updatedNote){
 const reserved=getOrderReservedItems(o);
 if(o.stockRestored)return {restored:false,reason:'already-restored'};
 if(!reserved.length)return {restored:false,reason:'not-reserved'};
 if(configured){
   await runTransaction(db,async tx=>{
     const or=doc(db,'orders',id),os=await tx.get(or);if(!os.exists())throw Error('Order not found.');const live=os.data();if(live.stockRestored)throw Error('STOCK_ALREADY_RESTORED');
     const rows=getOrderReservedItems(live);if(!rows.length){tx.update(or,{status:'Cancelled',pharmacistNote:updatedNote,cancelledAt:serverTimestamp(),updatedAt:serverTimestamp(),stockRestoreStatus:'No reserved stock to restore'});return}
     const batchTotals=new Map(),productTotals=new Map();
     for(const it of rows){const q=Math.max(0,Number(it.qty??it.quantity??0));const pid=it.productId||'';const bid=it.batchId||'';if(q<=0||!pid||!bid)throw Error('Reserved order item is missing productId or batchId.');batchTotals.set(bid,(batchTotals.get(bid)||0)+q);productTotals.set(pid,(productTotals.get(pid)||0)+q)}
     const refs=[...batchTotals.keys()].map(x=>doc(db,'batches',x)).concat([...productTotals.keys()].map(x=>doc(db,'products',x)));
     const snaps=await Promise.all(refs.map(r=>tx.get(r)));let n=0;
     for(const [bid,q] of batchTotals){const snap=snaps[n++];if(!snap.exists())throw Error('Batch not found for stock restore: '+bid);tx.update(doc(db,'batches',bid),{stock:Number(snap.data().stock||0)+q,updatedAt:serverTimestamp()})}
     for(const [pid,q] of productTotals){const snap=snaps[n++];if(!snap.exists())throw Error('Product not found for stock restore: '+pid);tx.update(doc(db,'products',pid),{stock:Number(snap.data().stock||0)+q,updatedAt:serverTimestamp()})}
     for(const it of rows)tx.set(doc(collection(db,'stockMovements')),{type:'ORDER_CANCEL_RESTOCK',productId:it.productId,batchId:it.batchId,batchNumber:it.batchNumber||'',qty:Math.max(0,Number(it.qty??it.quantity??0)),reference:live.orderNumber||id,createdAt:serverTimestamp()});
 ...
 const batchTotals=new Map(),productTotals=new Map();for(const it of reserved){const q=Math.max(0,Number(it.qty??it.quantity??0)),pid=it.productId||'',bid=it.batchId||'';if(q<=0||!pid||!bid)throw Error('Reserved order item is missing productId or batchId.');batchTotals.set(bid,(batchTotals.get(bid)||0)+q);productTotals.set(pid,(productTotals.get(pid)||0)+q)}
 for(const [bid,q] of batchTotals){const b=batches.find(x=>x.id===bid);if(!b)throw Error('Batch not found for stock restore: '+bid);b.stock=Number(b.stock||0)+q}
 for(const [pid,q] of productTotals){const p=products.find(x=>x.id===pid);if(!p)throw Error('Product not found for stock restore: '+pid);p.stock=Number(p.stock||0)+q}
 Object.assign(o,{status:'Cancelled',pharmacistNote:updatedNote,cancelledAt:new Date().toISOString(),stockRestored:true,stockRestoredAt:new Date().toISOString(),stockRestoreStatus:'Stock automatically restored after admin cancellation'});const sm=get('stockMovements',[]);reserved.forEach(it=>sm.push({id:'SM'+Date.now()+Math.random(),type:'ORDER_CANCEL_RESTOCK',productId:it.productId,batchId:it.batchId,batchNumber:it.batchNumber||'',qty:Math.max(0,Number(it.qty??it.quantity??0)),reference:o.orderNumber||id,createdAt:new Date().toISOString()}));set('stockMovements',sm);set('batches',batches);set('products',products);set('orders',currentOrders);renderAll();return {restored:true};
}
window.cancelOrder=async id=>{
 ...
 if(o.status==='Cancelled')return alert(o.stockRestored?'Order already cancelled. Stock was already restored.':'Order is already cancelled.');
 if(!confirm('Cancel this customer order? If this order had reserved/deducted stock, the same stock will be automatically restored.'))return;
 try{
   ...
   if(result.restored)alert('Order cancelled successfully. Reserved stock has been automatically restored.');
   else if(configured){await updateDoc(doc(db,'orders',id),{status:'Cancelled',pharmacistNote:updatedNote,cancelledAt:serverTimestamp(),updatedAt:serverTimestamp(),stockRestoreStatus:'No reserved stock was deducted, so no restock was needed'});alert('Order cancelled successfully. No stock had been reserved/deducted, so no restock was needed.');}
   else{Object.assign(o,{status:'Cancelled',pharmacistNote:updatedNote,cancelledAt:new Date().toISOString(),stockRestoreStatus:'No reserved stock was deducted, so no restock was needed'});set('orders',currentOrders);renderOrders();alert('Order cancelled successfully. No stock had been reserved/deducted, so no restock was needed.');}
 }catch(e){if(e.message==='STOCK_ALREADY_RESTORED')alert('Order was already restored. Stock was not added twice.');else alert('Could not cancel order: '+e.message)}
};
window.updateOrder=async id=>{const existing=currentOrders.find(x=>x.id===id);if(existing&&existing.status==='Cancelled')return alert('Cancelled order cannot be changed or billed.');const st=$('st_'+id).value,note=$('note_'+id).value.trim();if(st==='Cancelled'){if(note)existing.pharmacistNote=note;return window.cancelOrder(id)}try{if(configured)await updateDoc(doc(db,'orders',id),{status:st,pharmacistNote:note,updatedAt:serverTimestamp()});else{const o=currentOrders.find(x=>x.id===id);if(o){o.status=st;o.pharmacistNote=note;set('orders',currentOrders)}}alert('Order status updated.')}catch(e){alert(e.message)}};

...
function renderStock(){const box=$('stockSummary');if(!box)return;const zero=products.filter(p=>Number(p.stock||0)<=0);const expiredProducts=products.filter(p=>batches.some(b=>b.productId===p.id&&expiryStatus(b)==='EXPIRED'));box.innerHTML='<div class="grid"><button class="secondary" type="button" onclick="setStockFilter(\'out\')">🔴 Out of stock: '+zero.length+'</button><button class="secondary" type="button" onclick="setStockFilter(\'expiry\')">⏰ Expired stock: '+expiredProducts.length+'</button></div><div class="stockFilterRow"><input id="stockSearch" value="'+esc(box.dataset.search||'')+'" oninput="filterStockSearch(this.value)" placeholder="🔍 Search medicine / barcode / batch"><button class="secondary smallBtn" type="button" onclick="setStockFilter(\'all\');$(\'stockSummary\').dataset.search=\'\';renderStock()">↩ All</button></div><div class="small" id="stockShowing" style="margin-top:8px"></div>';renderStockListOnly();}

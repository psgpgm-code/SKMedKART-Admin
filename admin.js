...
window.refreshPurchaseSearch=()=>renderPurchases();document.addEventListener('input',e=>{if(e.target&&e.target.id==='purchaseSearch')renderPurchases()});document.addEventListener('keyup',e=>{if(e.target&&e.target.id==='purchaseSearch')renderPurchases()});
...
function renderPurchases(){const input=$('purchaseSearch'),list=$('purchases');if(!list)return;const q=String(input?.value||'').trim().toLowerCase();const rows=(Array.isArray(purchases)?purchases:[]).filter(p=>{if(!q)return true;const text=[p.productName,p.medicine,p.name,p.supplier,p.supplierName,p.invoice,p.invoiceNo,p.purchaseInvoiceNo,p.batchNumber,p.batch,p.expiryDate,p.category,p.cat].map(v=>String(v??'')).join(' ').toLowerCase();return text.includes(q)}).slice(0,100);list.innerHTML=rows.map(p=>'<div class="itemrow"><b>'+esc(p.productName||p.medicine||p.name||'-')+'</b> • Qty '+Number(p.qty||0)+'<br><span class="small">'+esc(p.supplier||p.supplierName||'-')+' • Batch '+esc(p.batchNumber||p.batch||'-')+' • Exp '+esc(p.expiryDate||'-')+' • Purchase ₹'+Number(p.purchasePrice||0).toFixed(2)+' + GST '+Number(p.purchaseGstRate||0).toFixed(2)+'% = ₹'+Number(p.purchasePriceWithGst??(Number(p.purchasePrice||0)*(1+Number(p.purchaseGstRate||0)/100))).toFixed(2)+'</span></div>').join('')||'<div class="small">No purchases found.</div>'}

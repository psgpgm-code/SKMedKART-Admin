...
window.shareBill=async id=>{const b=findBill(id);if(!b)return;const text='Sri Krishna Medicals, Pennagaram\nInvoice: '+b.invoiceNumber+'\nCustomer: '+b.customerName+'\nTotal: '+money(b.grandTotal)+'\nPayment: '+b.paymentMode;try{if(typeof navigator.share==='function'){const file=new File([pdfBlob(b)],b.invoiceNumber+'.pdf',{type:'application/pdf'});if(typeof navigator.canShare==='function'&&navigator.canShare({files:[file]}))await navigator.share({title:'Medical Bill '+b.invoiceNumber,text,files:[file]});else await navigator.share({title:'Medical Bill '+b.invoiceNumber,text});return}}catch(e){if(e&&e.name==='AbortError')return}const wa='https://wa.me/?text='+encodeURIComponent(text);try{window.location.assign(wa)}catch(e){try{window.open(wa,'_blank','noopener')}catch(_){}}setTimeout(()=>{try{navigator.clipboard?.writeText(text)}catch(_){}},500)};

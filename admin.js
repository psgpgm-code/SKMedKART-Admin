...
function showMedicineSuggestions(inputId,boxId,onPick){
 ...
 const render=()=>{
   ...
   box.innerHTML=list.map(p=>{const availableStock=batches.filter(b=>b.productId===p.id&&Number(b.stock||0)>0&&expiryStatus(b)!=='EXPIRED').reduce((sum,b)=>sum+Number(b.stock||0),0);return '<div class="medicineOption" data-id="'+esc(p.id)+'"><b>'+esc(p.name)+'</b><small>Available stock: '+availableStock+'</small></div>'}).join('');

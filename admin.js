...
window.shareBill=async id=>{const b=findBill(id);if(!b)return alert('Bill not found.');const text='Sri Krishna Medicals, Pennagaram\nInvoice: '+b.invoiceNumber+'\nCustomer: '+b.customerName+'\nTotal: '+money(b.grandTotal)+'\nPayment: '+b.paymentMode;try{if(typeof navigator!=='undefined'&&typeof navigator.share==='function'){const file=new File([pdfBlob(b)],(b.invoiceNumber||'bill')+'.pdf',{type:'application/pdf'});if(typeof navigator.canShare==='function'&&navigator.canShare({files:[file]})){await navigator.share({title:'Medical Bill '+b.invoiceNumber,text,files:[file]});return}await navigator.share({title:'Medical Bill '+b.invoiceNumber,text});return}}catch(e){if(e?.name==='AbortError')return}try{if(navigator.clipboard?.writeText)await navigator.clipboard.writeText(text)}catch(e){}try{window.location.href='https://wa.me/?text='+encodeURIComponent(text)}catch(e){alert('Bill details copied. You can paste and send to the customer.')}};
